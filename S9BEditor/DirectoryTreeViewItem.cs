		...
		if (((TreeViewItem)this).IsExpanded)
		{
			...
			try
			{
				foreach (string item3 in Directory.EnumerateDirectories(ActualPath, "*", SearchOption.TopDirectoryOnly))
				{
					...
					{
						if (item4 is DirectoryTreeViewItem { IsDirectory: not false } directoryTreeViewItem2 && DataUtil.PathsEqual(directoryTreeViewItem2.ActualPath, item3))
						{
							flag = true;
							break;
						}
				...
				foreach (string item5 in Directory.EnumerateFiles(ActualPath, "*", SearchOption.TopDirectoryOnly))
				{
					...
					{
						if (item6 is DirectoryTreeViewItem { IsDirectory: false } directoryTreeViewItem4 && DataUtil.PathsEqual(directoryTreeViewItem4.ActualPath, item5))
						{
							flag2 = true;
							break;
						}
